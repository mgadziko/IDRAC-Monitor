param([string] $ConfigPath = 'C:\ProgramData\BlackLotusTelemetry\token.txt')

$ErrorActionPreference = 'Stop'
$token = (Get-Content -LiteralPath $ConfigPath -Raw).Trim()
if ([string]::IsNullOrWhiteSpace($token)) { throw 'Telemetry token is empty.' }
...
try {
    while ($listener.IsListening) {
        ...
        try {
            if ($context.Request.HttpMethod -ne 'GET' -or $context.Request.Url.AbsolutePath -ne '/v1/gpu-temperatures' -or $context.Request.Headers['X-BlackLotus-Token'] -ne $token) {
